...
export function registerGameHandlers(io: Server, socket: Socket) {
  ...
  socket.on('play_card', (payload: PlayCardPayload) => {
    ...
    const validation = GameEngine.validateMove(match, playerId, payload.cardId, hand);
    ...
    const { state: newState, mustPickup, pickedUpBy } = GameEngine.applyMove(
      match,
      playerId,
      card,
      allHands,
    ...
    const eliminated = GameEngine.checkEliminations(newState, allHands);
    ...
    const { isOver, thullaPlayerId } = GameEngine.checkMatchEnd(newState);
    if (isOver) {
...
function scheduleTurnTimeout(io: Server, roomId: string, playerId: string, match: MatchState) {
  ...
  const timer = setTimeout(() => {
    ...
    const hand = roomManager.getPlayerHand(match.matchId, playerId);
    const { forceCard, forcePickup } = GameEngine.handleTurnTimeout(match, hand);

    io.to(roomId).emit('turn_timeout', { playerId, forceCard, forcePickup });

    if (forceCard) {
      const allHands = roomManager.getAllHands(match.matchId) as Record<string, Card[]>;
      const { state: newState } = GameEngine.applyMove(match, playerId, forceCard, allHands);
      room.match = newState;
      ...
      const result = GameEngine.checkMatchEnd(newState);
      if (result.isOver) {
      ...
      const allHands = roomManager.getAllHands(match.matchId) as Record<string, Card[]>;
      const newState = GameEngine.forcePickup(match, playerId, allHands);
      room.match = newState;
      updateRoomHands(room, match.matchId, allHands, io);
      const eliminated = GameEngine.checkEliminations(newState, allHands);
      io.to(roomId).emit('card_played', {
...
function scheduleAITurn(
  ...
  setTimeout(() => {
    ...
    if ('pickup' in decision) {
      ...
      match.currentTurnPlayerId = GameEngine.nextPlayer(match, aiPlayer.id);
      match.turnStartedAt = Date.now();
      updateRoomHands(room, match.matchId, allHands, io);
      const eliminated = GameEngine.checkEliminations(match, allHands);
      ...
      const result = GameEngine.checkMatchEnd(match);
      if (result.isOver) {
    ...
    const { state: newState } = GameEngine.applyMove(match, aiPlayer.id, card, allHands);
    room.match = newState;
    updateRoomHands(room, match.matchId, allHands, io);
    const eliminated = GameEngine.checkEliminations(newState, allHands);
