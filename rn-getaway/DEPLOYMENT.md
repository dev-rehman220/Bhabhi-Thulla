...
1. Start the server on the host computer from `rn-getaway/server` with `npm start`.
2. Allow Node.js through the host computer firewall on TCP port `3001`.
3. Set `EXPO_PUBLIC_SERVER_URL` in `rn-getaway/.env` to the host computer LAN IPv4 address, such as `http://192.168.1.20:3001`.
